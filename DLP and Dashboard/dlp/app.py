import sys
import os
sys.path.append(os.path.abspath("../../Jailbreak Detection System"))
...
async def redact(req: RedactRequest):
    ...
    # Jailbreak Detection
    if detector:
        detection_result = detector.detect(req.text)
        if detection_result['is_jailbreak']:
            event = audit.log("jailbreak_blocked", direction=req.source, confidence=detection_result['confidence'])
            await broadcast(event)
            raise HTTPException(403, f"Jailbreak detected with confidence {detection_result['confidence']:.2f}")
...
async def validate(req: ValidationRequest):
    ...
    if detector:
        detection_result = detector.detect(req.text)
        if detection_result['is_jailbreak']:
            event = audit.log("jailbreak_blocked", direction="output", confidence=detection_result['confidence'])
            await broadcast(event)
            raise HTTPException(403, f"Jailbreak detected with confidence {detection_result['confidence']:.2f}")
...
async def process(req: ProcessRequest):
    ...
    if detector:
        detection_result = detector.detect(req.text)
        if detection_result['is_jailbreak']:
            event = audit.log("jailbreak_blocked", direction=req.direction, confidence=detection_result['confidence'])
            await broadcast(event)
            raise HTTPException(403, f"Jailbreak detected with confidence {detection_result['confidence']:.2f}")
